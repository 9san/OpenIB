...
$(document).ready(function() {
	var addukkohide = function() {
		...
		$(this).append(hr.ukkoboard);

		if (hiddenboards[board] !== true) {
			ukkohide.html(_("[hide]"));
			hr.hide();
		}
		else {
			ukkohide.html(_("[show]"));
			$(this).next().hide();
		}
		ukkohide.click(function() {
			...
			if (hiddenboards[board] !== true) {
					...
					find('.ukkohide').html(_("[hide]")).
					parent().find('hr').hide();
                	}
                	else {
                        	$('[data-board="'+board+'"]:not([data-cached="yes"])').hide().prev().
					find('.ukkohide').html(_("[show]"))
					.parent().find('hr').show();
                	}
			storeboards();
			return false;
	...
	var loadnext = function() {
		...
		while($(window).scrollTop() + $(window).height() + 1000 > $(document).height() && !loading && overflow.length > 0) {
			...
			var boardheader = $('<h2 class="ukkoboard"><a href="/' + overflow[0].board + '/">/' + overflow[0].board + '/</a></h2>');
